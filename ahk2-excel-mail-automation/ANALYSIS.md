...
| 전체 | F1 시작 / Esc 종료 | Main.ahk2 | 핫키 바인딩 |
...
ahk2-excel-mail-automation/
  Main.ahk2              엔트리포인트: 핫키(F1/Esc)만 정의, 실제 로직은 lib 참조
  lib/
    Config.ahk2           튜닝 가능한 값(대기시간, 창 제목, 단축키) - 유일한 설정 지점
    Logger.ahk2            파일 로그 (logs/ahk2_mail_YYYYMMDD.log)
    WaitUtil.ahk2          창 대기, 재시도(Retry) 공용 유틸
    ErrorHandler.ahk2      오류 발생 시 사용자 대응(건너뛰기/중단) 정책
    InputCollector.ahk2    2~8번: 입력 GUI + 검증 + 확인
    ExcelController.ahk2   1/9/10/14번: Excel COM 제어
    MailController.ahk2    11/12/13/15/17번: 메일 창 키보드 제어
    Workflow.ahk2          전체 오케스트레이션(9~17번)
```

**설계 원칙**
- `Config.ahk2` 외 다른 파일에는 매직넘버(대기시간, 창 제목 등)를 두지 않는다.
...
- `Workflow`만 두 컨트롤러를 조합한다 → 업무 순서가 바뀌면 `Workflow.ahk2`만 수정.
...
| 스크립트 전역 미처리 예외 | `Main.ahk2`의 try/catch가 최종 방어선 |
| Esc 입력 | 즉시 `ExitApp()` - 진행 중 COM 작업/메일 임시본이 남을 수 있음(아래 5번 참고) |

## 5. 대기시간 설계 (Config.ahk2)
...
7. **Esc 즉시 종료**: 요구사항대로 확인 없이 즉시 스크립트를 종료한다. 진행
   중이던 필터/작성 중이던 메일 임시 저장본은 그대로 남을 수 있다. 안전한
   되돌리기가 필요하면 종료 전 확인창을 추가하는 것을 권장한다(현재는 미적용).

## 7. 확장 포인트

- 다른 표/다른 메일 서비스에 재사용: `Config.ahk2`만 교체.
